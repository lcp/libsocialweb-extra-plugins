...
struct _BishoPaneMySpacePrivate {
...
#define GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), BISHO_TYPE_PANE_MYSPACE, BishoPaneMySpacePrivate))
G_DEFINE_DYNAMIC_TYPE (BishoPaneMySpace, bisho_pane_myspace, BISHO_TYPE_PANE);

static void update_widgets (BishoPaneMySpace *pane, ButtonState state);

static char *
create_url (BishoPaneMySpace *pane, const char *token)
{
  SoupURI *base, *uri;
  char *s;
...
request_token_cb (OAuthProxy   *proxy,
                  ...
                  gpointer      user_data)
{
  BishoPaneMySpace *pane = BISHO_PANE_MYSPACE (user_data);
  BishoPaneMySpacePrivate *priv = pane->priv;
...
static void
bisho_pane_myspace_continue_auth (BishoPane *_pane, GHashTable *params)
{
  BishoPaneMySpace *pane = BISHO_PANE_MYSPACE (_pane);
  BishoPaneMySpacePrivate *priv = pane->priv;
  ServiceInfo *info = BISHO_PANE (pane)->info;
  GError *error = NULL;
  const char *verifier;
...
static void
update_widgets (BishoPaneMySpace *pane, ButtonState state)
{
  BishoPaneMySpacePrivate *priv;
  ServiceInfo *info;

  g_assert (BISHO_IS_PANE_MYSPACE (pane));
  priv = pane->priv;
...
static const char *
bisho_pane_myspace_get_auth_type (BishoPaneClass *klass)
{
  return "myspace";
}

static void
bisho_pane_myspace_constructed (GObject *object)
{
  BishoPaneMySpace *pane = BISHO_PANE_MYSPACE (object);
  BishoPaneMySpacePrivate *priv = pane->priv;
  ServiceInfo *info = BISHO_PANE (pane)->info;
...
static void
bisho_pane_myspace_class_init (BishoPaneMySpaceClass *klass)
{
  GObjectClass *o_class = G_OBJECT_CLASS (klass);
  BishoPaneClass *pane_class = BISHO_PANE_CLASS (klass);

  o_class->constructed = bisho_pane_myspace_constructed;
  pane_class->get_auth_type = bisho_pane_myspace_get_auth_type;
  pane_class->continue_auth = bisho_pane_myspace_continue_auth;

  g_type_class_add_private (klass, sizeof (BishoPaneMySpacePrivate));
}

static void
bisho_pane_myspace_class_finalize (BishoPaneMySpaceClass *klass)
{
}

static void
bisho_pane_myspace_init (BishoPaneMySpace *pane)
{
  BishoPaneMySpacePrivate *priv;
...
{
  bisho_pane_myspace_register_type (module);
}
